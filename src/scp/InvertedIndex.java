...
Grado Informatica
Juan Bone Valls
73210074Q Alberto Sanmartin Martinez.
--------------------------------------------------------------- */

/*
procesar un fichero de forma concurrente
se coge el fichero, se divide en el numero de hilos y se hace seek con cada seccion para cada hilo
vigilar dominios solapados
*/
...
public class InvertedIndex {
    ...
    private HashMultimap<String, Long> Hash = HashMultimap.create();    // Hash Map con el Índice Invertido. multiples valores para la misma clave
    ...
    public void BuildIndex() {
        ...
        try {
            ...
            while((car = is.read())!=-1) {
                ...
                // cuando se añade un nuevo k-word y no esta completo el anterior
...
*/
    ...
    */
    ...
    public void SaveIndex(String outputDirectory) {
        ...
        // en cada dichero se guarda un conjunto de k-words, entre 1 y 1000
    ...
    public void SaveIndex2(String outputDirectory) {
      ...
      if (keySet.size()>DIndexMaxNumberOfFiles)
          ...
          numberOfFiles = keySet.size();
      // Bucle para distribuir el trabajo entre los hilos.
      int wArea = numberOfFiles / k;

      for (int i=0; i<k; i++) {
          
          int start = wArea * i + 1;
          int end = wArea * (i + 1);
          if(i == k-1) end = numberOfFiles;
          Runnable r = new Save(outputDirectory, start, end);
          t[i] = new Thread (r);
          t[i].start();
